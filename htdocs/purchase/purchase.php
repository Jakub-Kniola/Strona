<?php
    ...
    $statement = mysqli_prepare($connection, 'SELECT product_id, price FROM products WHERE product_id = ?');
    ...
    $statement = mysqli_prepare($connection, 'INSERT INTO purchase VALUES (NULL, ?, ?, ?, NULL)');

    mysqli_stmt_bind_param($statement, 'iii', $user_id, $product_id, $fetched_price);
    mysqli_stmt_execute($statement);
    mysqli_stmt_close($statement);
    mysqli_close($connection);

    success('Produkt został kupiony pomyślnie!');
