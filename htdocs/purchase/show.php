<?php
    ...
    $statement = mysqli_prepare($connection, 'SELECT purchase.purchase_id, purchase.product_id, products.title, products.subtitle, products.publisher, products.released, purchase.price as purchase_price, purchase.date as purchase_date FROM purchase INNER JOIN products ON purchase.product_id = products.product_id WHERE user_id = ?');
